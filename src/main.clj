...
(defn is-dash?          [c] (= c \-))
(defn is-colon?         [c] (= c \:))
(defn is-left-carrot?   [c] (= c \<))
(defn is-right-carrot?  [c] (= c \>))
(defn is-comma?         [c] (= c \,))
...
(defn get-token 
  ...
  (loop [current-state :START token-value ""]
	...
	(is-asteric?       in-char) (recur :INMULOP (str in-char token-value))
	(is-dash?          in-char) (recur :INMINUS   (str in-char token-value))
	(is-colon?         in-char) (recur :INASSIGNOP (str in-char token-value))
	(is-right-carrot?  in-char) (recur :INGRELOP (str in-char))
	(is-left-carrot?   in-char) (recur :INLRELOP (str in-char))
